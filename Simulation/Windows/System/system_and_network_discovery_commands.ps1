...
try {
    ...
    $commands = @('cmd.exe /c "ipconfig /all"','cmd.exe /c "route PRINT"','cmd.exe /c "netstat -ano"','cmd.exe /c "hostname"','cmd.exe /c "systeminfo"','cmd.exe /c "tracert 8.8.8.8"','cmd.exe /c "getmac"','cmd.exe /c "arp -a"','cmd.exe /c "net share"','cmd.exe /c "net use"','cmd.exe /c "fsutil fsinfo drives"','cmd.exe /c "net view"','cmd.exe /c "gpresult /R /Z"','cmd.exe /c "net accounts"','cmd.exe /c "net group"','cmd.exe /c "WMIC /Node:localhost /Namespace:\\root\SecurityCenter2 Path AntiVirusProduct Get displayName /Format:List"','powershell.exe "Get-CimInstance -Namespace root/securityCenter2 –classname antivirusproduct"','cmd.exe /c "netsh advfirewall firewall show rule name=all dir=out type=dynamic"','cmd.exe /c "ver"','cmd.exe /c "vssadmin list shadows"','powershell.exe "Get-ComputerInfo"',' cmd.exe /c "nbtstat -s"','cmd.exe /c "nbtstat -n"','cmd.exe /c "net config workstation"','cmd.exe /c "netsh wlan show profiles"','cmd.exe /c "net session"',"whoami",'cmd.exe /c "query user"','cmd.exe /c "net start"')
