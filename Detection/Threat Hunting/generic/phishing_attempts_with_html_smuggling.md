...
### manually opening a downloaded PAYMENTS.html from an archive with winrar:
  - **Registry**:
    - Sysmon EID 12 & 13 with process `WinRAR.exe` and target registry key containing `*\\Explorer\\*.html*`
   - **File created** when opening the html page from the archive (Sysmon EID 11):
    - from process `*\\WinRAR.exe` and target file name `*\\Users\\*\\AppData\\Local\\Temp\\Rar*\\PAYMENTS.html`
  - **CommandLines** from parent process `*\\WinRAR.exe` (Security EID 4688 or Sysmon EID 1) :
